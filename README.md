...
| 模块 | 进度 | 状态说明 |
|------|------|----------|
| **Web 端**（`packages/web`） | 95% | 登录/注册/找回密码、文档上传与阅读、评论回复点赞、目录与阅读设置、**通知系统**已可用 |
| **API**（`packages/api`） | 98% | JWT 鉴权 + bcrypt 密码哈希已完成、OTP、文档处理队列、评论 SSE、跨文档评论聚合、**通知 API**已配置 |
...
| **Docker 部署** | 98% | 本地与生产 compose 均可启动，支持 Postgres/Redis/Qdrant/API/Worker/Web；环境变量验证已添加 |
...
| **用户通知系统** | 评论回复/@自动通知，铃铛组件 + 消息页面 | ✅ 完成 |
| **消息中心** | `/profile/messages` 筛选/批量操作 | ✅ 完成 |
...
| `/profile/messages` | 消息中心（通知列表、筛选、批量操作） |
...
- ✅ Swagger API 文档配置
- ✅ 环境变量验证与启动检查
- ✅ **用户通知系统**（2026-03-07）- 数据库/API/前端组件/自动触发/部署验证
- ✅ **消息中心页面**（2026-03-07）- `/profile/messages` 筛选/批量操作
...
4. **Telegram 推送扩展** - 通知系统集成 Telegram bot（已配置）
