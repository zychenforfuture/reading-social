...
export default function RegisterPage() {
  ...
  const sendCodeMutation = useMutation({
    mutationFn: () => api.sendCode(email, 'register'),
    onSuccess: () => {
      setCountdown(60);
  ...
  const registerMutation = useMutation({
    mutationFn: () => api.register(email, username, password, code),
    onSuccess: () => {
  ...
  const errorMsg = (() => {
    ...
    const msg = (registerMutation.error as any)?.message || (sendCodeMutation.error as any)?.message || '';
    if (msg.includes('验证码错误')) return '验证码错误，请重新输入';
    if (msg.includes('已过期')) return '验证码已过期，请重新发送';
    if (msg.includes('已注册')) return '该邮箱已注册，请直接登录';
    return msg || '';
  })();
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-md p-8 space-y-5">
        ...
        {sendCodeMutation.isSuccess && !registerMutation.isError && (
